...
function _handleServerAddresses(DNSservers) {
  ...
  if (typeof DNSservers === 'string') {
    if ((" " in DNSservers)) return DNSservers.split(' ');
      ...
      throw "A space must be in DNSservers if it's a string";
      return;
    }
  }
...
function _checkVars({DNSservers, DNSbackupName, loggingEnable, mkBackup}) {
  ...
  if (typeof DNSservers === 'object') DNSservers.map((i) => {
    if (typeof i !== 'string' || (/^(?!0)(?!.*\.$)((1?\d?\d|25[0-5]|2[0-4]\d)(\.|$)){4}$/.test(i) !== true)) {
      throw "DNSservers[*] each must be strings and valid IPv4 addresses";
      return;
    }
  });
